...
//	    			document.getElementById('content').textContent = evt.target.result;
    				...
    				var gridDimen = width * height;
 					var strtSolutionGrid = parseInt("0x33")+1;
    				var endSolutionGrid = parseInt("0x33")+gridDimen;
    				var solutionGrid = extract(content,strtSolutionGrid,endSolutionGrid);    				
//    				document.write(stringFromCode(solutionGrid) + "<br>");
...
//    					document.write(infoAndClues.substr(locArInfoAndClues[i],(locArInfoAndClues[i+1]-locArInfoAndClues[i])) + "<br>");
...
//    				document.write(title + "<br>" + author + "<br>" + copyright + "<br>");
    				var clues = ArInfoAndClues.slice(3,-2);
//    				document.write(clues + "<br>");
    			//add a surrounded by black
    				
					var matBlank = fillbGrid(bG,width,height);
					var reqMatBlank = getReqMatBlank(matBlank,width,height);
//    				document.write(matBlank[0][0] + "<br>");
			...
			for(var y=0;y<=h;y++){	
				if (mat[x][y] == '.'){
					...
					if((mat[x][y+1] == '-') && (mat[x+1][y] == '-')){
						if((mat[x][y-1] == '.') && (mat[x-1][y] == '.')){
							num++;	
							nmat[x][y] = '-'  + num + 'AD'; //across and down
							cN.push(num + 'AD');
...
//				document.write(nmat[x][y] + " ");
			}// end of for y
//			document.write("<br>");
		}//end  of for x
//		document.write(cN + "<br>");
	...
	function buildMap(reqnumMat,width,height) {
		...
		for (var x = 0; x < height; x++) {
			...
			for (var y = 0; y < width; y++) { 				
	            ...
	            var cell = { x:y, y:x, type:type, num:num,dir:dir};
	            map.grid[x][y] = cell;
	            map[type].push(cell);
	        }
	...
	function clueHighlight(data,width,height) {
		var clueCells = {grid: [],across: [],down:[],acrossdown: []};		
		for (var x = 0; x < height; x++) {
			clueCells.grid[x] = [];			
			for (var y = 0; y < width; y++) { 
		...
		var gridGroup = svgContainer.append("g");
	  	...
	  	if(cssClass == "white"){
	  	    ...
	  	    var textAttributes = text
	                             .attr("x", function (d) { return scales.x(d.x + 0.05); })
	                             .attr("y", function (d) { return scales.y(d.y + 0.2); })
...
/*		for(i=0;i<=across.length;i++){
			a.innerHTML=across[i];
			document.write(across[i] + "<br>");
		}
